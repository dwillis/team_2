...
#Because we have to monitor the incidents to tweet them out, we have have to keep running the code for an extended period of time to collect many instances Also, it has to refresh the requests every 30 seconds (which is the data rate of refresh). But we also have to consider limits to the API Because the incidents don't stay in the system we created a list that appends every time new data comes in.
